...
# Split the fastq file into the given partitions
		...
		if selected_part_assem == "SPAdes":
			for p in range(1, num_parts + 1):
				...
				os.popen(spades_path + "/spades.py -t " + str(t) + " -m " + str(m) + " -k " + p_kmer + " --12 " + out_path + "/" + ntpath.basename(partitions_fastq_file)[:-5] + "_part_" + str(p) +".fastq" + " -o " + out_path + "part_" + str(p) + "_assembly | tee -a " + out_path +  "HGA.log").read()
